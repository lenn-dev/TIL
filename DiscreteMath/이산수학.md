...
실수는 A,B 가 둘다 0 이 아닐 경우 곱한 값이 0이 나올 수 없지만, 행렬은 A, B 가 둘 다 영행렬이 아닌 경우에도 행렬의 곱이 영행렬이 될 수 있다. 
...
'최단 거리 테이블' 은 1차원 배열로, n개 노드까지 오는데 필요한 최단거리를 기록한다. n개 (1부터 시작하는 노드 번호와 일치시키려면 n+1개 ) 크기의 배열을 선언하고 큰 값(무한대)을 넣어 초기화 시킨다.

'노드 방문 여부 체크 배열'은 방문한 노드인지 아닌지 기록하기 위한 배열로, 크기는 '최단 거리 테이블'과 같다. 인접 노드 거리가 __모두__ 업데이트 되면 방문 체크 된다. 기본적으로는 `false`로 초기화하여 방문하지 않았음을 명시한다.

G 그래프, W 가중치(weight), S 시작점, d[v] vertex 까지의 distance 값
...
- 출발노드 S 는 1번, 도착노드는 6번
- 거리 테이블을 전부 큰 값, 여기서는 inf(무한대)로 초기화 함.
- 각 노드들 잇는 간선의 가중치 W 역시 표시
- d[v] : vertex 까지의 distance 
- d[u] : 현재 노드 까지의 distance
- 방문하지 않은 Q={}
- Q 중 인접한 vertex 는 Q'
...
- 출발 노드 S 선택하고, 거리를 0으로 함.
- 방문하지 않은 Q = {1,2,3,4,5,6}

<img width="556" alt="Screenshot 2024-04-26 at 7 13 59 PM" src="https://github.com/lenn-dev/TIL/assets/37726487/1d1690b1-fac2-4749-8b47-26ff10d536f1">


- 1번 노드와 연결된 인접 노드는 2, 4
- 두 인접 노드로 가는 거리를 초기값(inf)과 비교해 둘 다 최소값으로 업데이트 해야 함, min() 함수사용
- 목표노드까지의 거리 = 현재위치까지거리 + (현재위치~목표위치까지의 거리)
- d[목표노드] = d[현재위치한 노드] + w(현재위치,목표위치) 
- d[2]까지 거리,  d [2] = d[1] + w(1,2) = 0 + 2 = 2
- d[4]까지 거리,  d [4] = d[1] + w(1,4) = 0 + 1 = 1
- min() 함수 사용해 초기값과 계산값 비교해 더 작은 값 업데이트 하기 :
- d[2] = min(inf, 2) = 2  => 업데이트
- d[4] = min(inf, 1)= 1  => 업데이트
- 인접 노드(2,4)까지 거리를 모두 업데이트한 1번 노드는 방문표시함 => True,
- 방문하지 않은 Q = {2,3,4,5,6}
- 최근 갱신한 테이블 기준(2,4)으로, 방문하지 않은 노드 중 가장 거리값이 작은 번호를 다음 노드로 택함, 위 상태에서는 4번 노드 선택.
  
<img width="554" alt="Screenshot 2024-04-26 at 7 25 51 PM" src="https://github.com/lenn-dev/TIL/assets/37726487/b5927c41-8b19-47fd-ae1e-abb46eff7380">

- 4번 노드에서 같은 작업 수행, 4번과 인접한 (방문처리된 1번 노드를 뺀) 2,5 번 노드까지의 거리를 계산
- d[2]까지 거리,  d [2] = d[4] + w(4,2) = 1 + 2 = 3
- d[5]까지 거리,  d [5] = d[4] + w(4,5) = 1 + 1 = 2
- min() 함수 사용해 기존값과 계산값 비교해 더 작은 값 업데이트 하기 :
- d[2] = min(2, 3) = 2  => 유지
- d[5] = min(inf, 2)= 2  => 업데이트
- ~~인접 노드(2,5)까지 거리를 모두 업데이트한 4번 노드는 방문표시함 => True,~~
- ~~방문하지 않은 Q = {2,3,5,6}~~
- 최근 갱신한 테이블 기준(2,5)으로, 방문하지 않은 노드 중 가장 거리값이 작은 번호를 다음 노드로 택함, 위 상태에서는 2번 노드 선택.

- 업데이트된 2,5 의 거리값이 2로 같은데, '거리 값이 같다면 인덱스가 작은 노드를 선택한다' 가정하고 2번 선택한다.

<img width="551" alt="Screenshot 2024-04-26 at 7 57 47 PM" src="https://github.com/lenn-dev/TIL/assets/37726487/d416613e-9c60-4ec9-923a-078e33fc7e82">


- 2번 노드와 연결된 3,4 번 노드에 대해 같은 과정 반복 (~~4번은 지나왔는데 왜 또함??~~)
- 2번까지 오는 최단거리는 1번에서 2번으로 바로 오는 거리인 2
- d[3]까지 거리,  d [3] = d[2] + w(2,3) = 2 + 3 = 5
- d[4]까지 거리,  d [4] = d[2] + w(2,4) = 2 + 2 = 4
- min() 함수 사용해 기존값과 계산값 비교해 더 작은 값 업데이트 하기 :
- d[3] = min(inf, 5) = 5  => 업데이트
- d[4] = min(1, 4)= 1     => 유지
- 작은 거리값인 4번 노드 선택
- 인접 노드(3,4)까지 거리를 모두 업데이트한 2번 노드는 방문표시함 => True,
- 방문하지 않은 Q = {3,5,6}
- 최근 갱신한 테이블 기준(3,4)으로, 방문하지 않은 노드 중 가장 거리값이 작은 번호를 다음 노드로 택함, 위 상태에서는 4번 노드 선택.
- d[5]까지 거리,  d [5] = d[4] + w(4,5) = 1 + 1 = 2
- min() 함수 사용해 기존값과 계산값 비교해 더 작은 값 업데이트 하기 :
- d[5] = min(2, 2) = 2  => 유지
- 4번 노드에서 방문처리된 1,2 제외하고 남은 노드 중 5번 노드 선택


<img width="543" alt="Screenshot 2024-04-26 at 8 08 31 PM" src="https://github.com/lenn-dev/TIL/assets/37726487/e057df43-1e79-42e9-9601-f9e44dec3d45">

- 5번 노드와 연결된 6번 노드에 같은 과정 반복
- d[6]까지 거리,  d [6] = d[5] + w(5,6) = 2 + 2 = 4
- min() 함수 사용해 기존값과 계산값 비교해 더 작은 값 업데이트 하기 :
- d[6] = min(inf, 4) = 4  => 업데이트
- 아직 방문하지 않은 3,6번 중 거리값이 작은 6번
- 도착노트 이므로 알고리즘 종료

<img width="557" alt="Screenshot 2024-04-26 at 8 14 01 PM" src="https://github.com/lenn-dev/TIL/assets/37726487/17c4f41e-0fee-4af0-b062-80b487889c6d">


- 최종적으로 1번에서 6번까지 오는데 1-4-5-6 경로를 거치고 최소거리는 4가 된다.
- 다익스트라 알고리즘은 방문하지 않은 노드 중 최단거리인 노드를 선택하는 과정을 반복한다.
- 또한 각 단계마다 탐색노드로 한 번 선택된 노드는 최단거리를 갱신하고, __그 뒤에는 더 작은 값으로 다시 갱신되지 않는다.__
...
## 1. 기본 계수법칙
- 경우의 수 : 어떤 사건이 일어나는 방법의 가짓수
- 계수문제 : 경우의 수를 세는 문제

### 1) 곱의 법칙(multiplication rule) : N(A×B) = N(A) × N(B) = m × n
- 두 사건 A,B가 일어날 경우의 수가 각각 N(A)= m, N(B)= n 일때, 사건 A,B가 동시에 일어날 경우의 수는 m × n 이다.
- 8비트로 표현할 수 있는 2진수는 몇 개? 2<sup>8</sup> = 256개 (8비트 각 자리 2진수 2가지 경우의 수 = 2*2*2*2*2*2*2*2)

### 2) 합의 법칙(additional rule) : N(A∪B) = N(A) + N(B) = n+m
- 두 사건 A,B가 일어날 경우의 수가 각각 N(A)=n, N(B)=m 이고 A∩B=∅일때, 사건 A 또는 B가 일어날 경우의 수는 n+m 이다
3) 원소의 개수가 2 또는 3인 집합에 대한 포함/제외 규칙
   N(A∪B) = N(A) + N(B) - N(A∩B)
...
## 2. 순열
### 1) 순열(permutation) :
- 객체의 집합에서 순서를 고려하여 배열하는 것
- 1 ≤ r ≤ n을 만족하는 정수 n, r에 대하여, n개의 원소를 갖는 집합에서 r개 원소를 순서를 고려하여 뽑는 경우의 수는
             
### 2) 중복집합의 순열(permutation of multiset) :
- 중복된 원소를 허용하는 중복집합의 크기가 n이고 그 중에서 중복된 원소의 개수가 각각 p개, q개, ⋯, r개가 있을때, n개 모두를 일렬로 배열하는 경우의 수는
             
### 3) 중복순열(repeated permutation) :
- n개의 원소를 갖는 집합에서 중복을 허용하여 r개 원소를 순서를 고려하여 뽑는 경우의 수는 n∏r = nr

### 4) 원순열(circular permutation) :
- n개의 원소를 갖는 집합의 모든 원소들을 원형으로 나열하는 경우의 수는
             

## 3. 조합
### 1) 조합(combination) :
- 객체의 집합에서 일부를 선택하는 것
- 0 ≤ r ≤ n을 만족하는 정수 n, r에 대하여, n개의 원소를 갖는 집합에서 r개 원소를 순서없이 뽑는 경우의 수는
             
### 2) 이항정리(binomial theorem) :
- 임의의 실수 x, y와 음이 아닌 정수 n이 주어졌을때,
             

## 4. 이산확률
- 확률 : 어떤 사건이 일어날 것인지 혹은 일어났는지에 대한 가능성을 표현
  
### 1) 표본공간과 사건
어떤 실험을 하였을때, 가능한 모든 결과 중에서 반드시 하나의 결과만 나타난다고 하자.
- 표본공간(sample space) : 실험의 모든 결과의 집합
- 사건(event) : 표본공간의 부분집합
  
### 2) 수학적 확률(mathematical probability) :
- 표본공간 S가 유한하며, 각각의 결과들은 나타날 가능성이 모두 동일하다고 했을때, S의 부분집합 사건 E가 나타날 확률은
             
이때, 표본공간 S의 모든 원소 s에 대하여 0 ≤ P(s) ≤ 1과 P(∅)=0, P(S)=1 이 성립한다.
표본공간 S의 사건 A,B에 대해서는 만약 A,B가 서로소이면 P(A∪B) = P(A)+P(B)가 성립한다.
...
- 수열의 항 사이에서 성립하는 관계식
- 수열 a1, a2, ⋯ , an-1, an⋯ 에서 항 an이 그전에 항들(a1, a2, ⋯ , an-1)의 함수로 나타낼 경우, 그 함수를 점화식이라 한다
- 수열의 일반항인 을 n에 관한 식으로 나타내는 것
- 피보나치(Fibonacci) 수 :
토끼 개체 수의 증가에 대해서 다음과 같은 간단한 가정을 하였다. 이를 식으로 표현한 것
➀ 첫 달에는 새로 태어난 토끼 한 쌍만이 존재한다
➁ 두 달 이상이 된 토끼는 번식이 가능하다
➂ 번식 가능한 토끼는 매달 한 쌍의 새끼를 낳는다
➃ 토끼는 죽지 않는다

## 6. 비둘기집 원리 (pigeonhole principle)
