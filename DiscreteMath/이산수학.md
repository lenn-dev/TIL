...
- 계수문제 : 경우의 수를 세는 문제
### 1) 곱의 법칙(multiplication rule) : N(A×B) = N(A) × N(B) = n×m
- 두 사건 A,B가 일어날 경우의 수가 각각 N(A)=n, N(B)=m 일때, 사건 A,B가 동시에 일어날 경우의 수는 n×m이다
### 2) 합의 법칙(additional rule) : N(A∪B) = N(A) + N(B) = n+m
- 두 사건 A,B가 일어날 경우의 수가 각각 N(A)=n, N(B)=m 이고 A∩B=∅일때, 사건 A 또는 B가 일어날 경우의 수는 n+m이다
